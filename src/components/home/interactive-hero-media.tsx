  ...
  useEffect(() => {
    ...
    const update = async () => {
      ...
      try {
        ...
        if (disposed || attempt !== generation || !capable()) { handle.dispose(); return; }
    ...
    <figure className="interactive-hero" aria-label="Explore the Glowbaby stroller light concept">
      ...
      <div className="interactive-hero-stage" onContextMenu={(event) => event.preventDefault()}>
        <div className="interactive-hero-render">
          <Image src={posters[mode]} alt="Close-up of the Glowbaby prototype beneath a stroller basket, casting colored light across a concrete sidewalk at dusk." fill draggable={false} sizes="(min-width: 1024px) 900px, calc(100vw - 2rem)" className={`interactive-hero-poster object-contain ${ready ? "is-hidden" : ""}`} />
        ...
        <span className="interactive-hero-badge">{ready ? "Drag to explore" : status === "loading" ? "Preparing 3D" : "2D preview"}</span>
      </div>
      <div className="interactive-hero-panel">
        ...
        <figcaption className="hero-media-caption">
          ...
          <span role="status">{status === "loading" ? "Loading 3D · Image remains available" : status === "error" ? "3D couldn’t load · Showing the 2D preview" : ready ? "Drag to explore · Scroll outside the view" : "2D preview · 3D loads automatically when supported"}</span>
        </figcaption>
      </div>
    </figure>
  );
