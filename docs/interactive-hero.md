...
The colored floor spill now has a subdued, desaturated center, broader coverage and feathered edges. Lower direct-light intensity and softer cones reduce concentrated color hotspots. These are visual treatments, not measured light output. Regenerate all three posters after changing the shared scene so the interactive and static versions stay consistent.
...
`hero-section.tsx` keeps the headline, copy, CTAs, and dedicated static 3D render server rendered. On narrow layouts its semantic order is copy then artwork; desktop places the image beside the copy without duplicating content or rearranging keyboard focus. `principles-section.tsx` places the interactive viewer beneath the **Visibility works both ways** introduction. `interactive-hero-media.tsx` owns fallback posters, semantic mode buttons, selected states, live descriptions and capability gating. `stroller-hero-scene.ts` owns lazy Three.js loading, assembly, lights, shadows, interaction, resize and resource cleanup. Three.js is the sole added runtime dependency. Development tools: glTF Transform core/extensions/functions, Meshoptimizer, Sharp and Three types.
...
`npm run hero:posters` captures and encodes every mode from the shared `lightModes` configuration plus the full-stroller static hero render; it never just re-encodes old PNGs. It starts an isolated loopback preview on an available port and an installed headless Edge/Chromium browser with a temporary private profile. Node.js 22.18+ is required; Edge and Chrome are detected on Windows, or set `HERO_BROWSER_PATH` to an installed Chromium executable. No browser or dependency is downloaded.

All three fresh 1200x1072 renders must succeed before any image is replaced. Model, texture, renderer and context failures invalidate capture readiness, including a failure occurring during the screenshot. Mode PNGs are saved to `.local-assets/stroller-render-MODE.png`, the static hero PNG to `.local-assets/stroller-hero-static.png`, and optimized WebPs to `public/hero/`. The command closes its own browser/server and removes its temporary profile. Rerun it whenever geometry, camera, materials or lighting change. Static image imports fingerprint the actual new bytes.
...
- `npm run hero:posters` loads and GPU-renders all actual models in each mode plus the full-stroller hero view, refusing shader or initialization errors. The current three images come from this command, not older captures.
