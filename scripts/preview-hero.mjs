...
<style>body{margin:0;background:#eeeafd;font:14px system-ui}#scene{width:100vw;height:calc(100vw / 1.12);max-height:90vh}nav{display:flex;gap:8px;justify-content:center;padding:12px}button{padding:12px 20px;border-radius:24px;border:1px solid #cbbce4;background:white}#status{text-align:center}body[data-capture] #scene{width:100vw;height:100vh;max-height:none}body[data-capture] nav,body[data-capture] #status{display:none}</style>
...
try{const handle=await createHeroScene(document.querySelector('#scene'),params.get('mode')||'flow',()=>{status.textContent='Rendering failed';});
handle.setActive(true);status.textContent='Actual Glowbaby CAD • local review only';
...
if(params.has('capture'))setTimeout(()=>{handle.setActive(false);document.body.dataset.ready='true';},3200);
}catch(error){status.textContent=error.message;console.error(error);}</script></html>`;

http.createServer(async (request, response) => {
  try {
    const url = new URL(request.url, "http://127.0.0.1:3010");
    const origin = request.headers.origin;
    if (origin && !["http://127.0.0.1:3010", "http://localhost:3010"].includes(origin)) { response.writeHead(403).end(); return; }
...
}).listen(3010, "127.0.0.1", () => console.log("Local-only hero preview: http://127.0.0.1:3010"));
